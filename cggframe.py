...
class CGGframe(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.category_embedding = nn.Embedding(num_categorys, embedding_dim, max_norm=1)
        self.category_indices = nn.Parameter(
            th.arange(num_categorys, dtype=th.long), requires_grad=False
        )
    ...
    def precompute_KG_embeddings(self):
        ...
        ft_device = self.category_indices.device
        ...
        with th.no_grad():
            ...
            feats = {
                'category': self.category_embedding(self.category_indices),
    ...
    def forward(self, inputs):
        if inputs is None:
            ...
            feats = {
                'category': self.category_embedding(used_nodes['category']),
