...
import torch as th
import dgl
from utils.data.transform import seq_to_unweighted_graph


def collate_fn_for_rnn_cnn(samples):
    uids, seqs, labels = zip(*samples)
    labels = th.LongTensor(labels)
    uids = list(range(1, 100))
...
def collate_fn_for_gnn_factory(samples):
    uids, seqs, labels = zip(*samples)
    uids = list(range(1, 100))
    uids = th.LongTensor(uids)
    labels = th.LongTensor(labels)
    ...
    inputs = [uids]
    for seq_to_graph in [seq_to_unweighted_graph]:
...
def sample_blocks(g, uniq_cids, uniq_iids, fanouts, steps):
    seeds = {'category': th.LongTensor(uniq_cids), 'item': th.LongTensor(uniq_iids)}
    blocks = []
...
class CollateFnGNN:
    def __init__(
        self, knowledge_graph, df_edges, num_layers, num_neighbors, seq_to_graph_fns, **kwargs
    ):
        self.knowledge_graph = knowledge_graph
        self.num_layers = num_layers
        self.seq_to_graph_fns = seq_to_graph_fns
        self.df_edges = np.array(df_edges.T).tolist()
        # num_neighbors is a list of integers
        if len(num_neighbors) != num_layers:
            assert len(num_neighbors) == 1
            self.fanouts = num_neighbors * num_layers
    ...
    def _collate_fn(self, samples, fanouts):
        seqs_ca, seqs, labels = zip(*samples)
        labels = th.LongTensor(labels)
        iids = np.concatenate(seqs)
        a_cids = np.concatenate(seqs_ca)

        new_iids, uniq_iids = pd.factorize(iids, sort=True)
        new_cids, uniq_cids = pd.factorize(a_cids, sort=True)
        ...
        cur_cidx = 0
        new_seqs_ca = []
        for i, seq_ca in enumerate(seqs_ca):
            new_seq_ca = new_cids[cur_cidx:cur_cidx + len(seq_ca)]
            cur_cidx += len(seq_ca)
            new_seqs_ca.append(new_seq_ca)
        ...
        for seq_to_graph in self.seq_to_graph_fns:
            graphs_ca = [seq_to_graph(seq_ca) for seq_ca in new_seqs_ca]
            bg_ca = dgl.batch(graphs_ca)
            inputs.append(bg_ca)

        extra_inputs = sample_blocks(
            self.knowledge_graph, uniq_cids, uniq_iids, fanouts, self.num_layers
        )
    ...
    def collate_test(self, samples):
        ...
        for seq_to_graph in self.seq_to_graph_fns:
            graphs_ca = [seq_to_graph(seq_ca) for seq_ca in seqs_ca]
            bg_ca = dgl.batch(graphs_ca)
            inputs.append(bg_ca)
...
class CollateFnGNN1:
    ...
    def _collate_fn(self, samples, fanouts):
        ...
        padded_seqs_ca = []
        for seq_ca in new_seqs_ca:
            padded_seq_ca = [-1] * 50
            padded_seq_ca[-len(seq_ca):] = seq_ca
            padded_seqs_ca.append(padded_seq_ca)

        padded_seqs = th.LongTensor(padded_seqs)
        padded_seqs_ca = th.LongTensor(padded_seqs_ca)
        pos = th.LongTensor([i for i in range(50)])

        inputs = []

        inputs.append(padded_seqs)
        inputs.append(padded_seqs_ca)
        inputs.append(pos)
