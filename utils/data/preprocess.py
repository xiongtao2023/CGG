...
def filter_isolated_categorys(df_clicks, df_edges):
    df_edges = df_edges[df_edges['itemId'].isin(df_clicks['itemId'])]
...
def filter_loop(df_clicks, df_edges, args):
    while True:
        ...
        df_conn, df_edges = filter_isolated_categorys(df_freq, df_edges)
...
def save_sessions(df_clicks, filepath):
    ...
    df_clicks.to_csv(filepath, sep='\t', header=['categorys', 'items'], index=True)


def keep_valid_sessions(df_train, df_test, train_split):
    print('\nprocessing test sets...')
    iid = df_train.itemId.unique()
    df_test = df_test[df_test.itemId.isin(iid)]
...
def print_stats(df_clicks, name):
    print(
        ...
        f'No. of category: {df_clicks.category.nunique()}\n'
...
def save_dataset(df_train, df_test, df_edges, df_loc, args):
    ...
    print(f'No. of ItemId: {df_edges.itemId.nunique()}')
    print(f'No. of Category: {df_edges.category.nunique()}')
    num_category = df_train.category.nunique()
    print(f'Avg. Categorys: {len(df_edges) / num_category:.3f}')

    df_train, df_test, df_edges = update_id(
        df_train, df_test, df_edges, colnames='itemId'
    )
    df_train, df_edges, df_test = update_id(
        df_train, df_edges, df_test, colnames='category'
    )
    ...
    # save edges
    df_edges = df_edges.sort_values(['itemId', 'category'])
    df_edges.to_csv(dataset_dir / 'edges.txt', sep='\t', header=True, index=False)

    # save stats
    num_categorys = df_train.category.nunique()
    num_items = df_train.itemId.nunique()
    with open(dataset_dir / 'stats.txt', 'w') as f:
        f.write('num_categorys\tnum_items\tmax_len\n')
        f.write(f'{num_categorys}\t{num_items}\t{args.max_len}')
...
def preprocess(df_clicks, df_edges, df_loc, args):
    ...
    if 'sessionId' in df_clicks.columns:
        ...
        sessionId = df_clicks.sessionId.astype(str)
