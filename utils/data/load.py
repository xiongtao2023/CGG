...
class AugmentedDataset:
    def __init__(self, df, read_sid=False, sort_by_length=True):
        if read_sid:
            df = df[['sessionId', 'categorys', 'items']]
        else:
            df = df[['categorys', 'items']]
...
def read_sessions(filepath):
    ...
    df['categorys'] = df['categorys'].apply(lambda x: [int(i) for i in x.split(',')])
...
def build_knowledge_graph(df_train, df_edges, do_count_clipping=True):
    print('building heterogeneous knowledge graph...')
    transits = Counter()
    transits_cate = Counter()
    relations = Counter()
    for _, row in df_train.iterrows():
        seq_cate = row['categorys']
        seq = row['items']
        transits_cate.update(zip(seq_cate, seq_cate[1:]))
        transits.update(zip(seq, seq[1:]))
    for _, row in df_edges.iterrows():
        itemId = [row['itemId']]
        category = [row['category']]
        relations.update(zip(itemId, category))
    prev_c, next_c = zip(*transits_cate.keys())
    prev_i, next_i = zip(*transits.keys())
    relate_i, relate_c = zip(*relations.keys())
    kg = dgl.heterograph({
        ('category', 'transitsto_cate', 'category'): (prev_c, next_c),
        ('item', 'relate', 'category'): (relate_i, relate_c),
        ('category', 'relatedby', 'item'): (relate_c, relate_i),
        ('item', 'transitsto', 'item'): (prev_i, next_i),
    })
    transit_cnts = np.array(list(transits.values()))
    transit_cate_cnts = np.array(list(transits_cate.values()))
    relations_cnts = np.array(list(relations.values()))
    transit_cnts = th.LongTensor(transit_cnts)
    transit_cate_cnts = th.LongTensor(transit_cate_cnts)
    relations_cnts = th.LongTensor(relations_cnts)

    kg.edges['transitsto'].data['cnt'] = transit_cnts
    kg.edges['transitsto_cate'].data['cnt'] = transit_cate_cnts
    kg.edges['relate'].data['cnt'] = relations_cnts
    kg.edges['relatedby'].data['cnt'] = relations_cnts
    return kg
